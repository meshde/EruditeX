...
class AnsSelect(object):
    def __init__(self, inp_hidden_states_dim, hid_dim=50):
        ...
        self.train = theano.function([q_sent_hid, ans_sent_hid, ans_node_hid,
                                      ans_parent_hid, ans_sib_hid, answer], [], updates=self.updates)
        self.predict = theano.function(
            [q_sent_hid, ans_sent_hid, ans_node_hid, ans_parent_hid, ans_sib_hid], prediction)
    ...
    def compute(self, q_sent_hid, ans_sent_hid, ans_node_hid, ans_parent_hid, ans_sib_hid):
        x = T.dot(self.W_q, q_sent_hid) + T.dot(self.W_ans_sent, ans_sent_hid)
            ...
            + T.dot(self.W_ans_sib, ans_sib_hid)
