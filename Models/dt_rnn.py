...
class DT_RNN(object):
    def __init__(self, dep_len=56, dim=50, word_vector_size=50,
        ...
        self.set_params(initialization)
        # self.theano_build()

    def set_params(self, initialization):
        initializer = nn_utils.get_initialization_function(initialization)

        self.W_x =  initializer(shape=(self.word_vector_size, self.dim))
        self.W_dep = initializer(shape=(self.dep_len, self.dim, self.dim))
    ...
    def get_graph_input(self):
        ...
        ## It is important for dep_tags to be a vector of integers (ivector),\
        ## or W[dep_tags[idy]] in inner_loop() gives the error:\ 
...
class DTNE_RNN(DT_RNN):
    ...
    def set_params(self):
        super().set_params()
        self.W_ne = initializer(shape=(self.dep_len, self.dim))
